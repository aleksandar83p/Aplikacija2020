...
export class ArticleService extends TypeOrmCrudService<Article>{
    ...
    async editFullArticle(articleId: number, data: EditArticleDto): Promise<Article | ApiResponse>{
        const existingArticle : Article = await this.article.findOne(articleId,{
            // ukljuci informacije o cenama i featurima
            relations: ['articlePrices', 'articleFeatures']  // povezi articlePrices sa articleId, sada mozemo da pristupimo ceni
        });
        ...
        // dodajemo novu cenu samo ako se cene razlikuju
        const newPriceString: string = Number(data.price).toFixed(2);  // toFixed, broj pretvaram u string sa 2 decimale 50 -> "50.00"
    ...
    async search(data: ArticleSearchDto): Promise<Article[]>{
        const builder = await this.article.createQueryBuilder("article"); //"article" iz baze
    
        builder.innerJoinAndSelect("article.articlePrices",
        "ap",
        // ovo nije primer najbolje prakse
        "ap.createdAt = (SELECT ap.createdAt FROM article_price AS ap WHERE ap.article_id = article.article_id ORDER BY ap.createdAt DESC LIMIT 1)");
